...
import { csrfStateCookieName } from "@root/config";
...
export const createNewAuthAccount = async (userId: number, {
    providerName, providerAccountId, email, avatarImage, accessToken, tokenType, refreshToken, authType, scope
}: AuthUserProfile) => {
...
export const getUserIpAddress = (ctx: Context) => {
    let ipAddr =
        ...
        (ctx.env.ip as string as IpAddressType);
