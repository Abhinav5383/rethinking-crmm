...
const getLoggedInSessions = async () => {
    try {
        ...
        const sessions: UserSession[] = (await response.json())?.sessions || [];
        return sessions;
    } catch (error) {
        console.error(error);
        return null;
...
const SessionsPage = () => {
    const [isLoading, setIsLoading] = useState<{ value: boolean; sessionId: number }>({ value: false, sessionId: 0 });
    ...
    const revokeSession = async (sessionId: number) => {
        try {
            ...
            setIsLoading({ value: false, sessionId: 0 });
        }
    };
                                          ...
                                          <div className="flex flex-col gap-2.5 sm:gap-1 grow">
                                              <div className="font-medium flex flex-wrap gap-x-2 items-center justify-start">
                                                  ...
                                                  <div className="flex gap-2 items-center justify-center">
                                                      ...
                                                      <CopyBtn text={session.ipAddress} id={`session-ip-${session.id}`} />
                                                  </div>
                                              </div>
                                              ...
                                              <div className="w-full flex flex-wrap gap-x-2 items-center justify-start text-muted-foreground">
                                                  ...
                                                  <Tooltip>
                                                      <TooltipTrigger className="cursor-text">
                                                          <span>Last used {timeSince(new Date(session.dateLastActive))}</span>
                                                      </TooltipTrigger>
