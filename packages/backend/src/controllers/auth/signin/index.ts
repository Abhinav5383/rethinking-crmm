...
export const oAuthSignInHandler = async (ctx: Context, authProvider: string, tokenExchangeCode: string) => {
    ...
    if (!profileData || !profileData?.email || !profileData?.providerName || !profileData?.providerAccountId) {
        return ctx.json({ message: "Invalid profile data received from the auth provider, most likely the code provided was invalid", success: false, received: profileData }, getHttpCode("bad_request"));
    }
    ...
    return ctx.json({ success: true, message: `Successfuly logged in using ${profileData.providerName} as ${expectedAuthAccount.user.fullName}` }, getHttpCode("ok"));
}
